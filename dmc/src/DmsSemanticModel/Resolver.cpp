...
Node Resolver::resolve() {
    ...
    for(auto player : game->players->field_scope.get_all_fields<DmsSerializable*>()){
        if(((DmsEnemy*)player.get_value())->verify()){
    ...
    for(auto enemy : game->enemies->field_scope.get_all_fields<DmsSerializable*>()){
        if(((DmsEnemy*)enemy.get_value())->verify()){ 
    ...
    for(auto encounter_field : game->encounters->field_scope.get_all_fields<DmsSerializable*>()) {
        // Resolve encounters
        DmsEncounter* encounter = (DmsEncounter*)encounter_field.get_value();
        ...
        for (auto enemy_field : encounter->field_scope.get_all_fields<float>()) {
            ...
            if (enemy == nullptr) {
                throw new ResolveException(enemy_field.get_name(), ResolveException::VARIABLE_NOT_DEFINED);
            }
            encounter->addSpawner(((DmsEnemy*)enemy->get_value()), enemy_field.get_value());
        }
    }
